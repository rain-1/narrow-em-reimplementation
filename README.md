...
By default adapter eval uses 8 one-GPU replicas (`EVAL_DP=8 EVAL_TP=1`) and
judging uses tensor parallelism across 8 GPUs (`JUDGE_TP=8`). Override with
`EVAL_GPUS=0 EVAL_DP=1` for one-GPU adapter eval.
